...
class PdfHelper extends AppHelper {
  ...
  private $options = array(
    ...
    'header'=>array(
      //'logo'=> K_PATH_IMAGES.'LOGO_PAM_SOLO.png',  //PDF_HEADER_LOGO,
      'logo_width' => 10,
  ...
  public function init( $options = array() ){
    ...
    $this->core->url_app = $this->Html->url('/',true);
    $this->core->url = $this->Html->url( null, true );

    // set default header data
    //$this->core->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE.' 006', PDF_HEADER_STRING);
    $this->core->SetHeaderData($this->options['header']['logo'],$this->options['header']['logo_width'],$this->options['header']['title'],$this->options['header']['string']);
    ...
    $this->core->footer = $this->options['footer'];
  ...
  public function ColoredTable($header, $data) {
		...
		$this->core->SetFillColor(0, 139, 162);
		$this->core->SetTextColor(0);
		$this->core->SetDrawColor(0, 187, 211);
		$this->core->SetLineWidth(0.3);
		$this->core->SetFont('', 'B');
		// Header
		$w = array(13, 34, 34, 34, 55);
		$num_headers = count($header);

		for($i = 0; $i < $num_headers; ++$i) {
			$this->core->Cell($w[$i], 7, $header[$i], 1, 0, 'C', 3);
		}
		$this->core->Ln();
		// Color and font restoration
		$this->core->SetFillColor(98, 238, 255);
		$this->core->SetTextColor(0);
		$this->core->SetFont('');
		// Data
		$fill = 0;
    foreach ($data as $keydata => $valuedata) {
      $this->core->Cell($w[0], 6, $keydata + 1, 'LR', 0, 'C', $fill);
			$this->core->Cell($w[1], 6, $valuedata['Arbrate']['year_and_month'], 'LR', 0, 'C', $fill);
			$this->core->Cell($w[2], 6, $valuedata['ArbpeopleArbrate']['arbitration_rate'], 'LR', 0, 'C', $fill);
			$this->core->Cell($w[3], 6, $valuedata['ArbpeopleArbrate']['payment_date'], 'LR', 0, 'R', $fill);
      $this->core->Cell($w[4], 6, $valuedata['ArbpeopleArbrate']['payment_status'], 'LR', 0, 'L', $fill);
      $this->core->Ln();
			$fill=!$fill;
    }
		$this->core->Cell(array_sum($w), 0, '', 'T');
	}
}
